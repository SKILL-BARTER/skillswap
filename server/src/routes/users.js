...
router.patch('/me', requireAuth, (req, res) => {
  ...
  // People often type "linkedin.com/in/…" or "github.com/…" without a
  // protocol — treat that as https:// rather than rejecting it, so the
  // frontend doesn't need strict browser-side URL validation either.
  const url = (v, fallback, max) => {
    if (v === undefined) return fallback;
  ...
  db.prepare(
    ...
    url(linkedin_url, current.linkedin_url, 300),
    url(github_url, current.github_url, 300),
    url(portfolio_url, current.portfolio_url, 300),
    req.user.id
  );
